...
public class QADSkinButton extends QADButton {
	...
	protected int entityRotation = 0;
	
	public QADSkinButton(String text, EnumNPCSkin skin) {
		super(text);
		npc = new EntityNPC(Minecraft.getMinecraft().theWorld);
		npc.getNPCData().setSkin(skin);
		EnumNPCModel model = skin.getModelType();
		if(model == null) model = EnumNPCModel.Player;
		npc.getNPCData().setModel(model);
	}
	...
	public void draw(int x, int y, float partial, VCUIRenderer renderer){
		...
		if(hovered)drawEntityOnScreen(renderer.getWidth() - 100, 150, 60, npc);
	}

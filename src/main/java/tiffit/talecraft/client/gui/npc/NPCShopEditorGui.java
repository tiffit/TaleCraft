...
public class NPCShopEditorGui extends QADGuiScreen {
	...
	private final MerchantRecipeList trades;
	
	public NPCShopEditorGui(NPCEditorGui npcGui) {
		this.setBehind(npcGui);
		this.returnScreen = npcGui;
		this.npcGui = npcGui;
		trades = npcGui.shop.getRecipes(Minecraft.getMinecraft().thePlayer);
	}
	...
	public void buildGui() {
		...
		addComponent(new QADButton(1, 1, this.width-52, new ButtonModel() {
			...
			public void onClick() {
				...
				trades.add(new MerchantRecipe(new ItemStack(item), null, new ItemStack(item), 0, Integer.MAX_VALUE));
		...
		for(int i = 0; i < trades.size(); i++){
			final int index = i;
			final MerchantRecipe trade = trades.get(index);
			for(int j = 0; j < 3; j++){
				final int type = j;
				ItemStack unchosenStack = null;
				switch(j){
				case 0: unchosenStack = trade.getItemToBuy();
						break;
				case 1: unchosenStack = trade.getSecondItemToBuy();
						break;
				case 2: unchosenStack = trade.getItemToSell();
						break;
				}
				final ItemStack stack = unchosenStack;
			...
			for(final ItemStack stack : items){
				...
				component.setAction( new Runnable() {
					@Override public void run() {
						...
						MerchantRecipe recipe = trades.get(index);
						...
						trades.remove(index);
						MerchantRecipe trade = new MerchantRecipe(buy1, buy2, sell, 0, Integer.MAX_VALUE);
						if(trades.size() < index) trades.add(index, trade);

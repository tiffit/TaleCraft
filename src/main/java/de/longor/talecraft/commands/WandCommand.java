...
public class WandCommand extends TCCommandBase {
	...
	private void execute_set(String[] args, EntityPlayerMP player, ICommandSender sender) throws CommandException {
		...
		if(args.length == 3) {
			...
			if(varStr.equalsIgnoreCase("x0")) {
				ix = (int) this.parseDouble(sender.getPosition().getX(), varStr, false);
			} else if(varStr.equalsIgnoreCase("y0")) {
				iy = (int) this.parseDouble(sender.getPosition().getY(), varStr, false);
			} else if(varStr.equalsIgnoreCase("z0")) {
				iz = (int) this.parseDouble(sender.getPosition().getZ(), varStr, false);
			} else if(varStr.equalsIgnoreCase("x1")) {
				ax = (int) this.parseDouble(sender.getPosition().getX(), varStr, false);
			} else if(varStr.equalsIgnoreCase("y1")) {
				ay = (int) this.parseDouble(sender.getPosition().getY(), varStr, false);
			} else if(varStr.equalsIgnoreCase("z1")) {
				az = (int) this.parseDouble(sender.getPosition().getZ(), varStr, false);
			} else {
			...
			int ix = (int) this.parseDouble(sender.getPosition().getX(), args[1], false);//bounds[0];
			int iy = (int) this.parseDouble(sender.getPosition().getY(), args[2], false);;//bounds[1];
			int iz = (int) this.parseDouble(sender.getPosition().getZ(), args[3], false);;//bounds[2];
			int ax = (int) this.parseDouble(sender.getPosition().getX(), args[4], false);;//bounds[3];
			int ay = (int) this.parseDouble(sender.getPosition().getY(), args[5], false);;//bounds[4];
			int az = (int) this.parseDouble(sender.getPosition().getZ(), args[6], false);;//bounds[5];
	...
	private void execute_expand_to(String[] args, EntityPlayerMP player, ICommandSender sender) throws CommandException {
		if(args.length == 4) {
			...
			int posX = (int) this.parseDouble(sender.getPosition().getX(), args[1], false);
			int posY = (int) this.parseDouble(sender.getPosition().getY(), args[2], false);
			int posZ = (int) this.parseDouble(sender.getPosition().getZ(), args[3], false);
	...
	public List addTabCompletionOptions(ICommandSender sender, String[] args, BlockPos pos) {
    	...
    	if(args[0].equals("region")) {
	    	...
	    	if(args.length == 3 && args[1].equals("fill")) {
	    		return this.getListOfStringsMatchingLastWord(args, Block.blockRegistry.getKeys());
	    	}
